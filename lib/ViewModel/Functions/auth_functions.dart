...
import '../../Helper/const_general.dart';
...
class AuthFunctions {
   static Future<void> updateToken() async {
       return await ConstState.fireStore.collection(fireStoreUser).doc(ConstState.firebaseId).update({
         'token': '${await ConstState.firebaseMessaging.getToken()}' ,
         'date': DateTime.now()
       });
  }
  ...
  static Future<void> loginAuth({
    ...
    try {
      ...
      if (globalKey.currentState!.validate()) {
        ...
        await ConstState.firebaseAuth.signInWithEmailAndPassword(
  ...
  static Future<void> resetEmailAuth({
    ...
    try {
      ...
      if (globalKey.currentState!.validate()) {
        ...
        await ConstState.firebaseAuth.sendPasswordResetEmail(email: email);
  ...
  static Future<void> changePassword({
    ...
    if(globalKey.currentState!.validate()) {
      ...
      return await ConstState.firebaseAuth.currentUser!.updatePassword(newPw)
          .then((value) async {
    ...
    return await showDialog(context: context, builder: (BuildContext context)=>AlertDialog(
      ...
      actions: [
        ...
        CustomElevatedButton(onPressed: ()async {
          await ConstState.fireStore.collection(fireStoreUser).doc(ConstState.firebaseId).update({
            'token': '' ,
          });
          await ConstState.firebaseAuth.signOut();
  ...
  static Future<void> _postUserData({required UserCredential user,required String first ,required String last}) async {
    ...
    await ConstState.fireStore.collection('User').doc(user.user!.uid).set(userModel.toApp());
  }



  static final fetchUserData = StreamProvider<DocumentSnapshot<Map<String,dynamic>>>((ref)  {
    return  ConstState.fireStore.collection(fireStoreUser).doc(ConstState.firebaseId).snapshots();
  });
  ...
  static Stream<DocumentSnapshot<Map<String,dynamic>>> getUserData() {
   ...
   static Stream<DocumentSnapshot<Map<String,dynamic>>> getUserDataById(String id) {
     return  ConstState.fireStore.collection(fireStoreUser).doc(id).snapshots();
   }

   static Future<QuerySnapshot<Map<String,dynamic>>> fetchAllUsers() async {
    return await ConstState.fireStore.collection(fireStoreUser).orderBy('date',descending: true).get();
   }

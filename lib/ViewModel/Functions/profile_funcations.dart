...
class FunctionsProfile {
  ...
  static Future<void> _uploadImage({required File file}) async {
    ...
    final Reference storage = ConstState.fireStorage.ref('$fireStoreUser/${ConstState.firebaseId}/$random${file.path.split('/').last}');
    final UploadTask getUrl = storage.putFile(file);
    image = await (await getUrl).ref.getDownloadURL();
    await ConstState.fireStore.collection(fireStoreUser).doc(ConstState.firebaseId).update({
      "image": image
    });
  }
  ...
  /// Uploaded Profile
    static Future<void> uploadedProfile({
      ...
      required ChangeNotifierProvider<SwitchState> provImage ,
